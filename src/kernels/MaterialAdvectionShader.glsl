...
void main() {
	...
	vec2 prevUV1 = getWrappedUV(vUV - 0.9 * displacement);//vec2(displacement.x, .y));
	vec2 prevUV2 = getWrappedUV(vUV - displacement);
	vec2 prevUV3 = getWrappedUV(vUV - 1.1 * displacement);//vec2(displacement.x, 1.1 * displacement.y));
